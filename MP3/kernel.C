...
int main() {
    ...
    //Made changes for getting frames 1 by one
    //Still need to grab first frame, as those after should be in order
	unsigned long process_mem_pool_info_frame = kernel_mem_pool.get_frames(n_info_frames);
	Console::putui(process_mem_pool_info_frame);
    for (int i = 1; i < n_info_frames; i++)
		Console::putui(kernel_mem_pool.get_frames(n_info_frames));
